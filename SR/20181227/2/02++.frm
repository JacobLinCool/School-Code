VERSION 5.00
Begin VB.Form Form1 
   Caption         =   "Form1"
   ClientHeight    =   8604
   ClientLeft      =   60
   ClientTop       =   516
   ClientWidth     =   10236
   LinkTopic       =   "Form1"
   ScaleHeight     =   8604
   ScaleWidth      =   10236
   StartUpPosition =   3  '系統預設值
   Begin VB.TextBox Text1 
      Height          =   264
      Left            =   9240
      TabIndex        =   146
      Text            =   "10"
      Top             =   600
      Width           =   852
   End
   Begin VB.CommandButton Command2 
      Caption         =   "Start"
      Height          =   612
      Left            =   8520
      TabIndex        =   144
      Top             =   960
      Width           =   1572
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   143
      Left            =   7440
      Style           =   1  '圖片外觀
      TabIndex        =   143
      Top             =   7200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   142
      Left            =   6840
      Style           =   1  '圖片外觀
      TabIndex        =   142
      Top             =   7200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   141
      Left            =   6240
      Style           =   1  '圖片外觀
      TabIndex        =   141
      Top             =   7200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   140
      Left            =   5640
      Style           =   1  '圖片外觀
      TabIndex        =   140
      Top             =   7200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   139
      Left            =   5040
      Style           =   1  '圖片外觀
      TabIndex        =   139
      Top             =   7200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   138
      Left            =   4440
      Style           =   1  '圖片外觀
      TabIndex        =   138
      Top             =   7200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   137
      Left            =   3840
      Style           =   1  '圖片外觀
      TabIndex        =   137
      Top             =   7200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   136
      Left            =   3240
      Style           =   1  '圖片外觀
      TabIndex        =   136
      Top             =   7200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   135
      Left            =   2640
      Style           =   1  '圖片外觀
      TabIndex        =   135
      Top             =   7200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   134
      Left            =   2040
      Style           =   1  '圖片外觀
      TabIndex        =   134
      Top             =   7200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   133
      Left            =   1440
      Style           =   1  '圖片外觀
      TabIndex        =   133
      Top             =   7200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   132
      Left            =   840
      Style           =   1  '圖片外觀
      TabIndex        =   132
      Top             =   7200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   131
      Left            =   7440
      Style           =   1  '圖片外觀
      TabIndex        =   131
      Top             =   6600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   130
      Left            =   6840
      Style           =   1  '圖片外觀
      TabIndex        =   130
      Top             =   6600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   129
      Left            =   6240
      Style           =   1  '圖片外觀
      TabIndex        =   129
      Top             =   6600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   128
      Left            =   5640
      Style           =   1  '圖片外觀
      TabIndex        =   128
      Top             =   6600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   127
      Left            =   5040
      Style           =   1  '圖片外觀
      TabIndex        =   127
      Top             =   6600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   126
      Left            =   4440
      Style           =   1  '圖片外觀
      TabIndex        =   126
      Top             =   6600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   125
      Left            =   3840
      Style           =   1  '圖片外觀
      TabIndex        =   125
      Top             =   6600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   124
      Left            =   3240
      Style           =   1  '圖片外觀
      TabIndex        =   124
      Top             =   6600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   123
      Left            =   2640
      Style           =   1  '圖片外觀
      TabIndex        =   123
      Top             =   6600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   122
      Left            =   2040
      Style           =   1  '圖片外觀
      TabIndex        =   122
      Top             =   6600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   121
      Left            =   1440
      Style           =   1  '圖片外觀
      TabIndex        =   121
      Top             =   6600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   120
      Left            =   840
      Style           =   1  '圖片外觀
      TabIndex        =   120
      Top             =   6600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   119
      Left            =   7440
      Style           =   1  '圖片外觀
      TabIndex        =   119
      Top             =   6000
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   118
      Left            =   6840
      Style           =   1  '圖片外觀
      TabIndex        =   118
      Top             =   6000
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   117
      Left            =   6240
      Style           =   1  '圖片外觀
      TabIndex        =   117
      Top             =   6000
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   116
      Left            =   5640
      Style           =   1  '圖片外觀
      TabIndex        =   116
      Top             =   6000
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   115
      Left            =   5040
      Style           =   1  '圖片外觀
      TabIndex        =   115
      Top             =   6000
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   114
      Left            =   4440
      Style           =   1  '圖片外觀
      TabIndex        =   114
      Top             =   6000
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   113
      Left            =   3840
      Style           =   1  '圖片外觀
      TabIndex        =   113
      Top             =   6000
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   112
      Left            =   3240
      Style           =   1  '圖片外觀
      TabIndex        =   112
      Top             =   6000
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   111
      Left            =   2640
      Style           =   1  '圖片外觀
      TabIndex        =   111
      Top             =   6000
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   110
      Left            =   2040
      Style           =   1  '圖片外觀
      TabIndex        =   110
      Top             =   6000
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   109
      Left            =   1440
      Style           =   1  '圖片外觀
      TabIndex        =   109
      Top             =   6000
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   108
      Left            =   840
      Style           =   1  '圖片外觀
      TabIndex        =   108
      Top             =   6000
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   107
      Left            =   7440
      Style           =   1  '圖片外觀
      TabIndex        =   107
      Top             =   5400
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   106
      Left            =   6840
      Style           =   1  '圖片外觀
      TabIndex        =   106
      Top             =   5400
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   105
      Left            =   6240
      Style           =   1  '圖片外觀
      TabIndex        =   105
      Top             =   5400
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   104
      Left            =   5640
      Style           =   1  '圖片外觀
      TabIndex        =   104
      Top             =   5400
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   103
      Left            =   5040
      Style           =   1  '圖片外觀
      TabIndex        =   103
      Top             =   5400
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   102
      Left            =   4440
      Style           =   1  '圖片外觀
      TabIndex        =   102
      Top             =   5400
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   101
      Left            =   3840
      Style           =   1  '圖片外觀
      TabIndex        =   101
      Top             =   5400
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   100
      Left            =   3240
      Style           =   1  '圖片外觀
      TabIndex        =   100
      Top             =   5400
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   99
      Left            =   2640
      Style           =   1  '圖片外觀
      TabIndex        =   99
      Top             =   5400
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   98
      Left            =   2040
      Style           =   1  '圖片外觀
      TabIndex        =   98
      Top             =   5400
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   97
      Left            =   1440
      Style           =   1  '圖片外觀
      TabIndex        =   97
      Top             =   5400
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   96
      Left            =   840
      Style           =   1  '圖片外觀
      TabIndex        =   96
      Top             =   5400
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   95
      Left            =   7440
      Style           =   1  '圖片外觀
      TabIndex        =   95
      Top             =   4800
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   94
      Left            =   6840
      Style           =   1  '圖片外觀
      TabIndex        =   94
      Top             =   4800
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   93
      Left            =   6240
      Style           =   1  '圖片外觀
      TabIndex        =   93
      Top             =   4800
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   92
      Left            =   5640
      Style           =   1  '圖片外觀
      TabIndex        =   92
      Top             =   4800
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   91
      Left            =   5040
      Style           =   1  '圖片外觀
      TabIndex        =   91
      Top             =   4800
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   90
      Left            =   4440
      Style           =   1  '圖片外觀
      TabIndex        =   90
      Top             =   4800
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   89
      Left            =   3840
      Style           =   1  '圖片外觀
      TabIndex        =   89
      Top             =   4800
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   88
      Left            =   3240
      Style           =   1  '圖片外觀
      TabIndex        =   88
      Top             =   4800
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   87
      Left            =   2640
      Style           =   1  '圖片外觀
      TabIndex        =   87
      Top             =   4800
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   86
      Left            =   2040
      Style           =   1  '圖片外觀
      TabIndex        =   86
      Top             =   4800
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   85
      Left            =   1440
      Style           =   1  '圖片外觀
      TabIndex        =   85
      Top             =   4800
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   84
      Left            =   840
      Style           =   1  '圖片外觀
      TabIndex        =   84
      Top             =   4800
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   83
      Left            =   7440
      Style           =   1  '圖片外觀
      TabIndex        =   83
      Top             =   4200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   82
      Left            =   6840
      Style           =   1  '圖片外觀
      TabIndex        =   82
      Top             =   4200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   81
      Left            =   6240
      Style           =   1  '圖片外觀
      TabIndex        =   81
      Top             =   4200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   80
      Left            =   5640
      Style           =   1  '圖片外觀
      TabIndex        =   80
      Top             =   4200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   79
      Left            =   5040
      Style           =   1  '圖片外觀
      TabIndex        =   79
      Top             =   4200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   78
      Left            =   4440
      Style           =   1  '圖片外觀
      TabIndex        =   78
      Top             =   4200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   77
      Left            =   3840
      Style           =   1  '圖片外觀
      TabIndex        =   77
      Top             =   4200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   76
      Left            =   3240
      Style           =   1  '圖片外觀
      TabIndex        =   76
      Top             =   4200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   75
      Left            =   2640
      Style           =   1  '圖片外觀
      TabIndex        =   75
      Top             =   4200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   74
      Left            =   2040
      Style           =   1  '圖片外觀
      TabIndex        =   74
      Top             =   4200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   73
      Left            =   1440
      Style           =   1  '圖片外觀
      TabIndex        =   73
      Top             =   4200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   72
      Left            =   840
      Style           =   1  '圖片外觀
      TabIndex        =   72
      Top             =   4200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   71
      Left            =   7440
      Style           =   1  '圖片外觀
      TabIndex        =   71
      Top             =   3600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   70
      Left            =   6840
      Style           =   1  '圖片外觀
      TabIndex        =   70
      Top             =   3600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   69
      Left            =   6240
      Style           =   1  '圖片外觀
      TabIndex        =   69
      Top             =   3600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   68
      Left            =   5640
      Style           =   1  '圖片外觀
      TabIndex        =   68
      Top             =   3600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   67
      Left            =   5040
      Style           =   1  '圖片外觀
      TabIndex        =   67
      Top             =   3600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   66
      Left            =   4440
      Style           =   1  '圖片外觀
      TabIndex        =   66
      Top             =   3600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   65
      Left            =   3840
      Style           =   1  '圖片外觀
      TabIndex        =   65
      Tag             =   "1"
      Top             =   3600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   64
      Left            =   3240
      Style           =   1  '圖片外觀
      TabIndex        =   64
      Top             =   3600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   63
      Left            =   2640
      Style           =   1  '圖片外觀
      TabIndex        =   63
      Top             =   3600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   62
      Left            =   2040
      Style           =   1  '圖片外觀
      TabIndex        =   62
      Top             =   3600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   61
      Left            =   1440
      Style           =   1  '圖片外觀
      TabIndex        =   61
      Top             =   3600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   60
      Left            =   840
      Style           =   1  '圖片外觀
      TabIndex        =   60
      Top             =   3600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   59
      Left            =   7440
      Style           =   1  '圖片外觀
      TabIndex        =   59
      Top             =   3000
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   58
      Left            =   6840
      Style           =   1  '圖片外觀
      TabIndex        =   58
      Top             =   3000
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   57
      Left            =   6240
      Style           =   1  '圖片外觀
      TabIndex        =   57
      Top             =   3000
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   56
      Left            =   5640
      Style           =   1  '圖片外觀
      TabIndex        =   56
      Top             =   3000
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   55
      Left            =   5040
      Style           =   1  '圖片外觀
      TabIndex        =   55
      Top             =   3000
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   54
      Left            =   4440
      Style           =   1  '圖片外觀
      TabIndex        =   54
      Top             =   3000
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   53
      Left            =   3840
      Style           =   1  '圖片外觀
      TabIndex        =   53
      Top             =   3000
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   52
      Left            =   3240
      Style           =   1  '圖片外觀
      TabIndex        =   52
      Top             =   3000
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   51
      Left            =   2640
      Style           =   1  '圖片外觀
      TabIndex        =   51
      Top             =   3000
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   50
      Left            =   2040
      Style           =   1  '圖片外觀
      TabIndex        =   50
      Top             =   3000
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   49
      Left            =   1440
      Style           =   1  '圖片外觀
      TabIndex        =   49
      Top             =   3000
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   48
      Left            =   840
      Style           =   1  '圖片外觀
      TabIndex        =   48
      Top             =   3000
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   47
      Left            =   7440
      Style           =   1  '圖片外觀
      TabIndex        =   47
      Top             =   2400
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   46
      Left            =   6840
      Style           =   1  '圖片外觀
      TabIndex        =   46
      Top             =   2400
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   45
      Left            =   6240
      Style           =   1  '圖片外觀
      TabIndex        =   45
      Top             =   2400
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   44
      Left            =   5640
      Style           =   1  '圖片外觀
      TabIndex        =   44
      Top             =   2400
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   43
      Left            =   5040
      Style           =   1  '圖片外觀
      TabIndex        =   43
      Top             =   2400
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   42
      Left            =   4440
      Style           =   1  '圖片外觀
      TabIndex        =   42
      Top             =   2400
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   41
      Left            =   3840
      Style           =   1  '圖片外觀
      TabIndex        =   41
      Top             =   2400
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   40
      Left            =   3240
      Style           =   1  '圖片外觀
      TabIndex        =   40
      Top             =   2400
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   39
      Left            =   2640
      Style           =   1  '圖片外觀
      TabIndex        =   39
      Top             =   2400
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   38
      Left            =   2040
      Style           =   1  '圖片外觀
      TabIndex        =   38
      Top             =   2400
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   37
      Left            =   1440
      Style           =   1  '圖片外觀
      TabIndex        =   37
      Top             =   2400
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   36
      Left            =   840
      Style           =   1  '圖片外觀
      TabIndex        =   36
      Top             =   2400
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   35
      Left            =   7440
      Style           =   1  '圖片外觀
      TabIndex        =   35
      Top             =   1800
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   34
      Left            =   6840
      Style           =   1  '圖片外觀
      TabIndex        =   34
      Top             =   1800
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   33
      Left            =   6240
      Style           =   1  '圖片外觀
      TabIndex        =   33
      Top             =   1800
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   32
      Left            =   5640
      Style           =   1  '圖片外觀
      TabIndex        =   32
      Top             =   1800
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   31
      Left            =   5040
      Style           =   1  '圖片外觀
      TabIndex        =   31
      Top             =   1800
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   30
      Left            =   4440
      Style           =   1  '圖片外觀
      TabIndex        =   30
      Top             =   1800
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   29
      Left            =   3840
      Style           =   1  '圖片外觀
      TabIndex        =   29
      Top             =   1800
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   28
      Left            =   3240
      Style           =   1  '圖片外觀
      TabIndex        =   28
      Top             =   1800
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   27
      Left            =   2640
      Style           =   1  '圖片外觀
      TabIndex        =   27
      Top             =   1800
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   26
      Left            =   2040
      Style           =   1  '圖片外觀
      TabIndex        =   26
      Top             =   1800
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   25
      Left            =   1440
      Style           =   1  '圖片外觀
      TabIndex        =   25
      Top             =   1800
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   24
      Left            =   840
      Style           =   1  '圖片外觀
      TabIndex        =   24
      Top             =   1800
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   23
      Left            =   7440
      Style           =   1  '圖片外觀
      TabIndex        =   23
      Top             =   1200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   22
      Left            =   6840
      Style           =   1  '圖片外觀
      TabIndex        =   22
      Top             =   1200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   21
      Left            =   6240
      Style           =   1  '圖片外觀
      TabIndex        =   21
      Top             =   1200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   20
      Left            =   5640
      Style           =   1  '圖片外觀
      TabIndex        =   20
      Top             =   1200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   19
      Left            =   5040
      Style           =   1  '圖片外觀
      TabIndex        =   19
      Top             =   1200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   18
      Left            =   4440
      Style           =   1  '圖片外觀
      TabIndex        =   18
      Top             =   1200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   17
      Left            =   3840
      Style           =   1  '圖片外觀
      TabIndex        =   17
      Top             =   1200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   16
      Left            =   3240
      Style           =   1  '圖片外觀
      TabIndex        =   16
      Top             =   1200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   15
      Left            =   2640
      Style           =   1  '圖片外觀
      TabIndex        =   15
      Top             =   1200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   14
      Left            =   2040
      Style           =   1  '圖片外觀
      TabIndex        =   14
      Top             =   1200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   13
      Left            =   1440
      Style           =   1  '圖片外觀
      TabIndex        =   13
      Top             =   1200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   12
      Left            =   840
      Style           =   1  '圖片外觀
      TabIndex        =   12
      Top             =   1200
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   11
      Left            =   7440
      Style           =   1  '圖片外觀
      TabIndex        =   11
      Top             =   600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   10
      Left            =   6840
      Style           =   1  '圖片外觀
      TabIndex        =   10
      Top             =   600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   9
      Left            =   6240
      Style           =   1  '圖片外觀
      TabIndex        =   9
      Top             =   600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   8
      Left            =   5640
      Style           =   1  '圖片外觀
      TabIndex        =   8
      Top             =   600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   7
      Left            =   5040
      Style           =   1  '圖片外觀
      TabIndex        =   7
      Top             =   600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   6
      Left            =   4440
      Style           =   1  '圖片外觀
      TabIndex        =   6
      Top             =   600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   5
      Left            =   3840
      Style           =   1  '圖片外觀
      TabIndex        =   5
      Top             =   600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   4
      Left            =   3240
      Style           =   1  '圖片外觀
      TabIndex        =   4
      Top             =   600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   3
      Left            =   2640
      Style           =   1  '圖片外觀
      TabIndex        =   3
      Top             =   600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   2
      Left            =   2040
      Style           =   1  '圖片外觀
      TabIndex        =   2
      Top             =   600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   1
      Left            =   1440
      Style           =   1  '圖片外觀
      TabIndex        =   1
      Top             =   600
      Width           =   615
   End
   Begin VB.CommandButton Command1 
      Height          =   615
      Index           =   0
      Left            =   840
      Style           =   1  '圖片外觀
      TabIndex        =   0
      Top             =   600
      Width           =   615
   End
   Begin VB.Label Label3 
      Caption         =   "Label3"
      Height          =   252
      Left            =   120
      TabIndex        =   148
      Top             =   8400
      Visible         =   0   'False
      Width           =   852
   End
   Begin VB.Label Label2 
      Caption         =   "0"
      Height          =   252
      Left            =   0
      TabIndex        =   147
      Top             =   0
      Width           =   372
   End
   Begin VB.Label Label1 
      Caption         =   "Count:"
      Height          =   252
      Left            =   8520
      TabIndex        =   145
      Top             =   600
      Width           =   732
   End
End
Attribute VB_Name = "Form1"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Private Sub Command1_Click(Index As Integer)
    '第一題
    If Command1(Index).Tag = "1" Then
        MsgBox "BOOM!"
        End
    Else
        Command1(Index).BackColor = &H8000000F
        Command1(Index).Caption = 0
        If Index Mod 12 <> 0 Then
            If Command1(Index - 1).Tag = "1" Then
                Command1(Index).Caption = Command1(Index).Caption + 1
            End If
        End If
        If Index Mod 12 <> 11 Then
            If Command1(Index + 1).Tag = "1" Then
                Command1(Index).Caption = Command1(Index).Caption + 1
            End If
        End If
        If Index Mod 12 <> 0 And Index > 11 Then
            If Command1(Index - 13).Tag = "1" Then
                Command1(Index).Caption = Command1(Index).Caption + 1
            End If
        End If
        If Index > 11 Then
            If Command1(Index - 12).Tag = "1" Then
                Command1(Index).Caption = Command1(Index).Caption + 1
            End If
        End If
        If Index Mod 12 <> 11 And Index > 11 Then
            If Command1(Index - 11).Tag = "1" Then
                Command1(Index).Caption = Command1(Index).Caption + 1
            End If
        End If
        
        If Index Mod 12 <> 11 And Index < 132 Then
            If Command1(Index + 13).Tag = "1" Then
                Command1(Index).Caption = Command1(Index).Caption + 1
            End If
        End If
        If Index < 132 Then
            If Command1(Index + 12).Tag = "1" Then
                Command1(Index).Caption = Command1(Index).Caption + 1
            End If
        End If
        If Index Mod 12 <> 0 And Index < 132 Then
            If Command1(Index + 11).Tag = "1" Then
                Command1(Index).Caption = Command1(Index).Caption + 1
            End If
        End If
        Label3 = 0
        For i = 0 To 143
            If Command1(i).Tag = "1" And Command1(i).BackColor = vbRed Then
                Label3 = Label3 + 1
                If Label3 = Text1 Then
                    MsgBox "WIN!"
                End If
            End If
        Next
    End If
End Sub

Private Sub Command1_MouseDown(Index As Integer, Button As Integer, Shift As Integer, X As Single, Y As Single)
    '第二題
    If Button = 2 Then  'button會傳回滑鼠狀態，1表示左鍵，2表右鍵，4表中鍵
        If Command1(Index).BackColor <> &H8000000F Then
            Command1(Index).BackColor = &H8000000F
            Command1(Index).Caption = ""
        ElseIf Command1(Index).Caption = "" Then
            Command1(Index).BackColor = vbRed
            Command1(Index).Caption = "?"
        End If
    End If
End Sub

Private Sub Command2_Click()
    For i = 1 To Text1 - 1
        a = Int(Rnd * 143)
        While Command1(a).Tag = "1"
            a = Int(Rnd * 143)
        Wend
        Command1(a).Tag = "1"
    Next
        For i = 0 To 143
            If Command1(i).Tag = "1" Then
                Label2 = Label2 + 1
            End If
        Next
    Command2.Caption = "Started"
End Sub

Private Sub Form_Load()
    Randomize
End Sub
