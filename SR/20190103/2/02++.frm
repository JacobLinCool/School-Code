...
Private Sub Command2_Click()
    ...
    End If
    Randomize
    Data = Split(InputBox("大小? (預設:10x10)", "大小?", "10x10"), "x")
    row = Data(1) + 2
    col = Data(0) + 2
    square = row * col
    For i = 0 To square - 1
    ...
    mine = InputBox("地雷數量? (預設:10)", "地雷數量?", 10)
    n = 0
    Do
        a = Int(Rnd * square)
    ...
    Command2.Left = Command1(col - 1).Left + Command1(col - 1).Width + 300
    Label1.Left = Command2.Left
    Me.Width = Label1.Left + Label1.Width + 400
    Me.Height = Command1(square - 1).Top + Command1(square - 1).Height + 400
    timeCost = 0
